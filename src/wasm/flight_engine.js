/* @ts-self-types="./flight_engine.d.ts" */
 ...
 * @param {number} years_per_trial
 * @param {number} seed
 * @returns {number}
 */
export function benchmark_montecarlo(num_trials, years_per_trial, seed) {
    const ret = wasm.benchmark_montecarlo(num_trials, years_per_trial, seed);
    return ret;
}
 ...
 * @returns {any}
 */
export function evaluate_policy(plan_js, assumptions_js, base_seed) {
    const ret = wasm.evaluate_policy(plan_js, assumptions_js, base_seed);
    if (ret[2]) {
 ...
 * @returns {any}
 */
export function simulate_trial(plan_js, assumptions_js, seed) {
    const ret = wasm.simulate_trial(plan_js, assumptions_js, seed);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
 ...
 * @returns {number}
 */
export function smoke_add(a, b) {
    const ret = wasm.smoke_add(a, b);
    return ret;
}
 ...
 * @returns {number}
 */
export function smoke_sum(values) {
    const ptr0 = passArrayF64ToWasm0(values, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.smoke_sum(ptr0, len0);
    return ret;
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('flight_engine_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
