...
} from '../src/chase-spending-import';
...
function parseArgs(argv: string[]): CliOptions {
  ...
  let outFile = 'public/local/spending-ledger.chase4582.json';
  ...
  if (inputFiles.length === 0) {
    throw new Error(
      'Usage: node --import tsx scripts/import-chase-spending.ts [--account chase-4582] [--out public/local/spending-ledger.chase4582.json] <csv...>',
    );
  }
...
function accountIdFromFile(filePath: string, fallback?: string): string | undefined {
  if (fallback) return fallback;
  const match = /Chase(\d+)/i.exec(path.basename(filePath));
  return match ? `chase-${match[1]}` : undefined;
}

function summarizeByMonth(
  transactions: ReturnType<typeof mergeSpendingImportTransactions>,
) {
...
async function main() {
  ...
  for (const filePath of options.inputFiles) {
    ...
    imports.push(
      importChaseActivityCsv(csvText, {
        fileName: path.basename(filePath),
        accountId: accountIdFromFile(filePath, options.accountId),
        importedAtIso,
      }),
    );
  }

  const transactions = mergeSpendingImportTransactions(imports);
