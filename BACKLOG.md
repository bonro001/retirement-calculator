...
- [ ] **Wire pre-retirement optimizer into flight-path policy + UI**. Engine module shipped in `src/pre-retirement-optimizer.ts`:
  - `buildPreRetirementOptimizerRecommendation(input)` returns `{ shortfalls, bridge, actionSteps, bothRecommendationsCompatible }` with concrete dollar amounts for 401(k) and HSA shortfalls, estimated current-year tax savings at the configured marginal rate, and a bridge-to-Medicare coverage projection that credits in-window windfalls (inheritance, home sale).
  - For the current seed, the demo test produces: 401(k) already maxed (100% funded, $21/yr shortfall — essentially noise), HSA at 35% funded (fix saves ~$1,366/yr fed tax), annual surplus after max ≈ $0, but bridge is **fully funded by $266k** because the $500k Dec 2028 inheritance lands in the bridge window. Concrete output replaces the generic "max 401k + pre-build taxable bridge" card that was actually partially wrong for this plan.
  - **Remaining adoption work**: (a) wire the recommendation into `src/flight-path-policy.ts` as a `pre_retirement_accumulation` candidate so it surfaces alongside other strategic-prep actions; (b) build a dashboard tile consuming the output (similar pattern to `TaxEfficiencyTile.tsx`). The engine signal is already decision-grade; just needs a UI home.
  - Files: `src/pre-retirement-optimizer.ts`, `src/pre-retirement-optimizer.test.ts`, `src/pre-retirement-optimizer-demo.test.ts`.
