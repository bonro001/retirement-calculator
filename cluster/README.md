...
| `HOST_ACCEPT_UPDATE_CONTROL` | follows `HOST_AUTO_UPDATE` | When `1`, a dispatcher `host_control` cycle command may force the same exit-75 update path even after the launcher temporarily disabled host-triggered auto-update. `scripts/start-rust-host.mjs` enables this for supervised hosts. |
...
| `CLUSTER_HOST_UPDATE_REQUEST_INTERVAL_MS` | `30000` | Dispatcher throttle for explicit mismatch cycle requests. Lower only while debugging a stuck worker. |
